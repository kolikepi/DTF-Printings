...
export default function CheckoutPage() {
  ...
  const [form, setForm] = useState({ shippingName: '', shippingAddress: '', shippingCity: 'Tiranë', shippingPhone: '', shippingNotes: '', paymentMethod: 'cash' });
  ...
  return (
    <div className="py-12">
      <div className="mx-auto max-w-[900px] px-4">
        <FadeIn>
          <h1 className="font-display text-3xl font-bold tracking-tight mb-8">{t('checkout.title')}</h1>
        </FadeIn>
              ...
              <Card className="p-6 sticky top-20">
                <CardContent>
                  ...
                  <div className="space-y-2 text-sm mb-4">
                    {(items ?? []).map((item: any) => (
                      <div key={item?.id} className="flex justify-between"><span className="truncate pr-2">{lang === 'sq' ? (item?.product?.nameAl ?? item?.product?.name) : (item?.product?.name ?? '')} x{item?.quantity ?? 1}</span><span>{(item?.product?.basePrice ?? 0) * (item?.quantity ?? 1)} {t('common.lek')}</span></div>
                    ))}
                  </div>
                  <div className="space-y-2 text-sm border-t pt-3">
                    <div className="flex justify-between"><span>{t('cart.subtotal')}</span><span>{subtotal} {t('common.lek')}</span></div>
                    <div className="flex justify-between"><span>{t('cart.shipping')}</span><span>{shipping === 0 ? (lang === 'sq' ? 'Falas' : 'Free') : `${shipping} ${t('common.lek')}`}</span></div>
                    <div className="border-t pt-2 flex justify-between font-bold text-lg"><span>{t('cart.total')}</span><span className="text-primary">{total} {t('common.lek')}</span></div>
                  </div>
